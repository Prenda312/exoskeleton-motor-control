#include "Robot_cal.h"
#ifndef Robot_MDLREF_HIDE_CHILD_
#include "Robot.h"
#endif                                 /*Robot_MDLREF_HIDE_CHILD_*/

/* Storage class 'PageSwitching' */
#ifndef Robot_MDLREF_HIDE_CHILD_

Robot_cal_type Robot_cal_impl = {
  /* Start of '<S219>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S255>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S255>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S255>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S219>/If Action Subsystem7' */

  /* Start of '<S218>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S252>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S252>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S252>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S218>/If Action Subsystem7' */

  /* Start of '<S217>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S249>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S249>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S249>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S217>/If Action Subsystem7' */

  /* Start of '<S216>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S246>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S246>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S246>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S216>/If Action Subsystem7' */

  /* Start of '<S215>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S243>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S243>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S243>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S215>/If Action Subsystem7' */

  /* Start of '<S214>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S238>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S238>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S238>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S214>/If Action Subsystem7' */

  /* Start of '<S213>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S230>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S230>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S230>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S213>/If Action Subsystem7' */

  /* Start of '<S151>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S187>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S187>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S187>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S151>/If Action Subsystem7' */

  /* Start of '<S150>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S184>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S184>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S184>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S150>/If Action Subsystem7' */

  /* Start of '<S149>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S181>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S181>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S181>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S149>/If Action Subsystem7' */

  /* Start of '<S148>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S178>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S178>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S178>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S148>/If Action Subsystem7' */

  /* Start of '<S147>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S175>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S175>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S175>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S147>/If Action Subsystem7' */

  /* Start of '<S146>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S170>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S170>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S170>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S146>/If Action Subsystem7' */

  /* Start of '<S145>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S162>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S162>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S162>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S145>/If Action Subsystem7' */

  /* Start of '<S83>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S119>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S119>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S119>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S83>/If Action Subsystem7' */

  /* Start of '<S82>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S116>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S116>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S116>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S82>/If Action Subsystem7' */

  /* Start of '<S81>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S113>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S113>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S113>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S81>/If Action Subsystem7' */

  /* Start of '<S80>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S110>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S110>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S110>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S80>/If Action Subsystem7' */

  /* Start of '<S79>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S107>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S107>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S107>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S79>/If Action Subsystem7' */

  /* Start of '<S78>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S102>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S102>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S102>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S78>/If Action Subsystem7' */

  /* Start of '<S77>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S94>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S94>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S94>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S77>/If Action Subsystem7' */

  /* Start of '<S19>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S54>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S54>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S54>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S19>/If Action Subsystem7' */

  /* Start of '<S18>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S51>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S51>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S51>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S18>/If Action Subsystem7' */

  /* Start of '<S17>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S48>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S48>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S48>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S17>/If Action Subsystem7' */

  /* Start of '<S16>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S45>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S45>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S45>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S16>/If Action Subsystem7' */

  /* Start of '<S15>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S42>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S42>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S42>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S15>/If Action Subsystem7' */

  /* Start of '<S14>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S37>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S37>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S37>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S14>/If Action Subsystem7' */

  /* Start of '<S13>/If Action Subsystem7' */
  {
    /* Computed Parameter: Out1_Y0
     * Referenced by: '<S29>/Out1'
     */
    0.0,

    /* Computed Parameter: DiscreteTimeIntegrator_gainval
     * Referenced by: '<S29>/Discrete-Time Integrator'
     */
    0.001,

    /* Expression: 0
     * Referenced by: '<S29>/Discrete-Time Integrator'
     */
    0.0
  }
  ,

  /* End of '<S13>/If Action Subsystem7' */

  /* Mask Parameter: State_1_Enable_TaskIndex
   * Referenced by: '<S67>/Constant2'
   */
  1.0,

  /* Mask Parameter: State_2_Enable_TaskIndex
   * Referenced by: '<S70>/Constant2'
   */
  2.0,

  /* Mask Parameter: State_3_Enable_TaskIndex
   * Referenced by: '<S73>/Constant2'
   */
  3.0,

  /* Mask Parameter: State_10_Enable_TaskIndex
   * Referenced by: '<S55>/Constant2'
   */
  10.0,

  /* Mask Parameter: State_11_Enable_TaskIndex
   * Referenced by: '<S58>/Constant2'
   */
  11.0,

  /* Mask Parameter: State_12_Enable_TaskIndex
   * Referenced by: '<S61>/Constant2'
   */
  12.0,

  /* Mask Parameter: State_13_Enable_TaskIndex
   * Referenced by: '<S64>/Constant2'
   */
  13.0,

  /* Mask Parameter: State_12_Enable_TaskIndex_g
   * Referenced by: '<S126>/Constant2'
   */
  12.0,

  /* Mask Parameter: State_1_Enable_TaskIndex_n
   * Referenced by: '<S132>/Constant2'
   */
  1.0,

  /* Mask Parameter: State_2_Enable_TaskIndex_o
   * Referenced by: '<S135>/Constant2'
   */
  2.0,

  /* Mask Parameter: State_3_Enable_TaskIndex_i
   * Referenced by: '<S138>/Constant2'
   */
  3.0,

  /* Mask Parameter: State_10_Enable_TaskIndex_b
   * Referenced by: '<S120>/Constant2'
   */
  10.0,

  /* Mask Parameter: State_11_Enable_TaskIndex_n
   * Referenced by: '<S123>/Constant2'
   */
  11.0,

  /* Mask Parameter: State_13_Enable_TaskIndex_m
   * Referenced by: '<S129>/Constant2'
   */
  13.0,

  /* Mask Parameter: State_9_Enable_TaskIndex
   * Referenced by: '<S141>/Constant2'
   */
  9.0,

  /* Mask Parameter: State_12_Enable_TaskIndex_n
   * Referenced by: '<S194>/Constant2'
   */
  12.0,

  /* Mask Parameter: State_1_Enable_TaskIndex_nj
   * Referenced by: '<S200>/Constant2'
   */
  1.0,

  /* Mask Parameter: State_2_Enable_TaskIndex_l
   * Referenced by: '<S203>/Constant2'
   */
  2.0,

  /* Mask Parameter: State_3_Enable_TaskIndex_b
   * Referenced by: '<S206>/Constant2'
   */
  3.0,

  /* Mask Parameter: State_10_Enable_TaskIndex_m
   * Referenced by: '<S188>/Constant2'
   */
  10.0,

  /* Mask Parameter: State_11_Enable_TaskIndex_p
   * Referenced by: '<S191>/Constant2'
   */
  11.0,

  /* Mask Parameter: State_13_Enable_TaskIndex_c
   * Referenced by: '<S197>/Constant2'
   */
  13.0,

  /* Mask Parameter: State_9_Enable_TaskIndex_c
   * Referenced by: '<S209>/Constant2'
   */
  9.0,

  /* Mask Parameter: State_12_Enable_TaskIndex_m
   * Referenced by: '<S262>/Constant2'
   */
  12.0,

  /* Mask Parameter: State_1_Enable_TaskIndex_l
   * Referenced by: '<S268>/Constant2'
   */
  1.0,

  /* Mask Parameter: State_2_Enable_TaskIndex_c
   * Referenced by: '<S271>/Constant2'
   */
  2.0,

  /* Mask Parameter: State_3_Enable_TaskIndex_p
   * Referenced by: '<S274>/Constant2'
   */
  3.0,

  /* Mask Parameter: State_10_Enable_TaskIndex_mi
   * Referenced by: '<S256>/Constant2'
   */
  10.0,

  /* Mask Parameter: State_11_Enable_TaskIndex_j
   * Referenced by: '<S259>/Constant2'
   */
  11.0,

  /* Mask Parameter: State_13_Enable_TaskIndex_g
   * Referenced by: '<S265>/Constant2'
   */
  13.0,

  /* Mask Parameter: State_9_Enable_TaskIndex_c4
   * Referenced by: '<S277>/Constant2'
   */
  9.0,

  /* Mask Parameter: State_1_Enable_TaskIndex_njg
   * Referenced by: '<S280>/Constant2'
   */
  1.0,

  /* Mask Parameter: State_2_Enable_TaskIndex_g
   * Referenced by: '<S283>/Constant2'
   */
  2.0,

  /* Mask Parameter: State_3_Enable_TaskIndex_a
   * Referenced by: '<S286>/Constant2'
   */
  3.0,

  /* Mask Parameter: State_4_Enable_TaskIndex
   * Referenced by: '<S289>/Constant2'
   */
  4.0,

  /* Mask Parameter: State_5_Enable_TaskIndex
   * Referenced by: '<S292>/Constant2'
   */
  5.0,

  /* Expression: 0
   * Referenced by: '<S13>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S13>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S14>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S14>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S15>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S15>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S16>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S16>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S17>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S17>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S18>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S18>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S19>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S19>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S2>/Memory1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S67>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S67>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S70>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S70>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S73>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S73>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S55>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S55>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S58>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S58>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S61>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S61>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S64>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S64>/Constant'
   */
  1.0,

  /* Computed Parameter: Merge1_InitialOutput
   * Referenced by: '<S2>/Merge1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S77>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S77>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S78>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S78>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S79>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S79>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S80>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S80>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S81>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S81>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S82>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S82>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S83>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S83>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S3>/Memory1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S126>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S126>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S132>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S132>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S135>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S135>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S138>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S138>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S120>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S120>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S123>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S123>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S129>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S129>/Constant'
   */
  1.0,

  /* Computed Parameter: Merge1_InitialOutput_k
   * Referenced by: '<S3>/Merge1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S141>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S141>/Constant1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S145>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S145>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S146>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S146>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S147>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S147>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S148>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S148>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S149>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S149>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S150>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S150>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S151>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S151>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S4>/Memory1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S194>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S194>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S200>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S200>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S203>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S203>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S206>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S206>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S188>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S188>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S191>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S191>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S197>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S197>/Constant'
   */
  1.0,

  /* Computed Parameter: Merge1_InitialOutput_l
   * Referenced by: '<S4>/Merge1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S209>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S209>/Constant1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S213>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S213>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S214>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S214>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S215>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S215>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S216>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S216>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S217>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S217>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S218>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S218>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S219>/Constant4'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S219>/Constant5'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S5>/Memory1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S262>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S262>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S268>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S268>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S271>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S271>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S274>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S274>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S256>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S256>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S259>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S259>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S265>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S265>/Constant'
   */
  1.0,

  /* Computed Parameter: Merge1_InitialOutput_j
   * Referenced by: '<S5>/Merge1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S277>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S277>/Constant1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<Root>/Memory1'
   */
  0.0,

  /* Expression: 0
   * Referenced by: '<S280>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S280>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<Root>/Constant'
   */
  0.0,

  /* Expression: 1000*1000
   * Referenced by: '<S295>/Gain1'
   */
  1.0E+6,

  /* Expression: 1000*1000
   * Referenced by: '<S297>/Gain1'
   */
  1.0E+6,

  /* Expression: 0
   * Referenced by: '<S283>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S283>/Constant'
   */
  1.0,

  /* Expression: 1000*1000
   * Referenced by: '<S300>/Gain1'
   */
  1.0E+6,

  /* Expression: 0
   * Referenced by: '<S286>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S286>/Constant'
   */
  1.0,

  /* Expression: 1000*1000
   * Referenced by: '<S296>/Gain1'
   */
  1.0E+6,

  /* Expression: 0
   * Referenced by: '<S289>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S289>/Constant'
   */
  1.0,

  /* Expression: 0
   * Referenced by: '<S292>/Constant1'
   */
  0.0,

  /* Expression: 1
   * Referenced by: '<S292>/Constant'
   */
  1.0,

  /* Computed Parameter: Merge1_InitialOutput_g
   * Referenced by: '<Root>/Merge1'
   */
  0.0,

  /* Computed Parameter: S1_AD_Ch1_Y0
   * Referenced by: '<S305>/S1_AD_Ch1'
   */
  0.0F,

  /* Computed Parameter: S1_AD_Ch2_Y0
   * Referenced by: '<S305>/S1_AD_Ch2'
   */
  0.0F,

  /* Computed Parameter: S1_Quaternions_q0_Y0
   * Referenced by: '<S305>/S1_Quaternions_q0'
   */
  0.0F,

  /* Computed Parameter: S1_Quaternions_q1_Y0
   * Referenced by: '<S305>/S1_Quaternions_q1'
   */
  0.0F,

  /* Computed Parameter: S1_Quaternions_q2_Y0
   * Referenced by: '<S305>/S1_Quaternions_q2'
   */
  0.0F,

  /* Computed Parameter: S1_Quaternions_q3_Y0
   * Referenced by: '<S305>/S1_Quaternions_q3'
   */
  0.0F,

  /* Computed Parameter: S2_AD_Ch1_Y0
   * Referenced by: '<S305>/S2_AD_Ch1'
   */
  0.0F,

  /* Computed Parameter: S2_AD_Ch2_Y0
   * Referenced by: '<S305>/S2_AD_Ch2'
   */
  0.0F,

  /* Computed Parameter: S2_Quaternions_q0_Y0
   * Referenced by: '<S305>/S2_Quaternions_q0'
   */
  0.0F,

  /* Computed Parameter: S2_Quaternions_q1_Y0
   * Referenced by: '<S305>/S2_Quaternions_q1'
   */
  0.0F,

  /* Computed Parameter: S2_Quaternions_q2_Y0
   * Referenced by: '<S305>/S2_Quaternions_q2'
   */
  0.0F,

  /* Computed Parameter: S2_Quaternions_q3_Y0
   * Referenced by: '<S305>/S2_Quaternions_q3'
   */
  0.0F,

  /* Computed Parameter: S3_AD_Ch1_Y0
   * Referenced by: '<S305>/S3_AD_Ch1'
   */
  0.0F,

  /* Computed Parameter: S3_AD_Ch2_Y0
   * Referenced by: '<S305>/S3_AD_Ch2'
   */
  0.0F,

  /* Computed Parameter: S3_Quaternions_q0_Y0
   * Referenced by: '<S305>/S3_Quaternions_q0'
   */
  0.0F,

  /* Computed Parameter: S3_Quaternions_q1_Y0
   * Referenced by: '<S305>/S3_Quaternions_q1'
   */
  0.0F,

  /* Computed Parameter: S3_Quaternions_q2_Y0
   * Referenced by: '<S305>/S3_Quaternions_q2'
   */
  0.0F,

  /* Computed Parameter: S3_Quaternions_q3_Y0
   * Referenced by: '<S305>/S3_Quaternions_q3'
   */
  0.0F,

  /* Computed Parameter: Gain8_Gain
   * Referenced by: '<S306>/Gain8'
   */
  78361.3359F,

  /* Computed Parameter: Bias_Bias
   * Referenced by: '<S306>/Bias'
   */
  48.0F,

  /* Computed Parameter: Gain8_Gain_h
   * Referenced by: '<S307>/Gain8'
   */
  81446.2266F,

  /* Computed Parameter: Bias_Bias_p
   * Referenced by: '<S307>/Bias'
   */
  44.0F,

  /* Computed Parameter: Gain8_Gain_p
   * Referenced by: '<S308>/Gain8'
   */
  74852.0156F,

  /* Computed Parameter: Bias_Bias_k
   * Referenced by: '<S308>/Bias'
   */
  10.0F,

  /* Computed Parameter: Gain8_Gain_n
   * Referenced by: '<S309>/Gain8'
   */
  75286.1641F,

  /* Computed Parameter: Bias_Bias_pl
   * Referenced by: '<S309>/Bias'
   */
  10.0F,

  /* Computed Parameter: Constant_Value_mh
   * Referenced by: '<S15>/Constant'
   */
  1,

  /* Computed Parameter: Constant1_Value_gd
   * Referenced by: '<S16>/Constant1'
   */
  1,

  /* Computed Parameter: Constant_Value_cn
   * Referenced by: '<S17>/Constant'
   */
  MAX_int32_T,

  /* Computed Parameter: Constant1_Value_pd
   * Referenced by: '<S17>/Constant1'
   */
  1,

  /* Computed Parameter: Constant2_Value
   * Referenced by: '<S18>/Constant2'
   */
  8,

  /* Computed Parameter: Constant2_Value_e
   * Referenced by: '<S19>/Constant2'
   */
  4,

  /* Computed Parameter: Constant_Value_k0
   * Referenced by: '<S79>/Constant'
   */
  1,

  /* Computed Parameter: Constant1_Value_ow
   * Referenced by: '<S80>/Constant1'
   */
  1,

  /* Computed Parameter: Constant_Value_n3i
   * Referenced by: '<S81>/Constant'
   */
  MAX_int32_T,

  /* Computed Parameter: Constant1_Value_d1
   * Referenced by: '<S81>/Constant1'
   */
  1,

  /* Computed Parameter: Constant2_Value_l
   * Referenced by: '<S82>/Constant2'
   */
  8,

  /* Computed Parameter: Constant2_Value_f
   * Referenced by: '<S83>/Constant2'
   */
  4,

  /* Computed Parameter: Constant_Value_pa
   * Referenced by: '<S147>/Constant'
   */
  1,

  /* Computed Parameter: Constant1_Value_kb
   * Referenced by: '<S148>/Constant1'
   */
  1,

  /* Computed Parameter: Constant_Value_ha
   * Referenced by: '<S149>/Constant'
   */
  MAX_int32_T,

  /* Computed Parameter: Constant1_Value_co
   * Referenced by: '<S149>/Constant1'
   */
  1,

  /* Computed Parameter: Constant2_Value_m
   * Referenced by: '<S150>/Constant2'
   */
  8,

  /* Computed Parameter: Constant2_Value_k
   * Referenced by: '<S151>/Constant2'
   */
  4,

  /* Computed Parameter: Constant_Value_fe
   * Referenced by: '<S215>/Constant'
   */
  1,

  /* Computed Parameter: Constant1_Value_jj
   * Referenced by: '<S216>/Constant1'
   */
  1,

  /* Computed Parameter: Constant_Value_kf
   * Referenced by: '<S217>/Constant'
   */
  MAX_int32_T,

  /* Computed Parameter: Constant1_Value_a
   * Referenced by: '<S217>/Constant1'
   */
  1,

  /* Computed Parameter: Constant2_Value_j
   * Referenced by: '<S218>/Constant2'
   */
  8,

  /* Computed Parameter: Constant2_Value_eo
   * Referenced by: '<S219>/Constant2'
   */
  4,

  /* Computed Parameter: M1_ActuralPosition_Y0
   * Referenced by: '<S305>/M1_ActuralPosition'
   */
  0,

  /* Computed Parameter: M1_ActuralVelocity_Y0
   * Referenced by: '<S305>/M1_ActuralVelocity'
   */
  0,

  /* Computed Parameter: M2_ActuralPosition_Y0
   * Referenced by: '<S305>/M2_ActuralPosition'
   */
  0,

  /* Computed Parameter: M2_ActuralVelocity_Y0
   * Referenced by: '<S305>/M2_ActuralVelocity'
   */
  0,

  /* Computed Parameter: M3_ActuralPosition_Y0
   * Referenced by: '<S305>/M3_ActuralPosition'
   */
  0,

  /* Computed Parameter: M3_ActuralVelocity_Y0
   * Referenced by: '<S305>/M3_ActuralVelocity'
   */
  0,

  /* Computed Parameter: M4_ActuralPosition_Y0
   * Referenced by: '<S305>/M4_ActuralPosition'
   */
  0,

  /* Computed Parameter: M4_ActuralVelocity_Y0
   * Referenced by: '<S305>/M4_ActuralVelocity'
   */
  0,

  /* Computed Parameter: Bias2_Bias
   * Referenced by: '<S301>/Bias2'
   */
  -3388710,

  /* Computed Parameter: Bias3_Bias
   * Referenced by: '<S304>/Bias3'
   */
  -9789179,

  /* Computed Parameter: Gain4_Gain
   * Referenced by: '<S301>/Gain4'
   */
  1342177280,

  /* Computed Parameter: Gain6_Gain
   * Referenced by: '<S298>/Gain6'
   */
  1342177280,

  /* Computed Parameter: Gain5_Gain
   * Referenced by: '<S304>/Gain5'
   */
  1342177280,

  /* Computed Parameter: Gain6_Gain_n
   * Referenced by: '<S302>/Gain6'
   */
  1342177280,

  /* Computed Parameter: Gain2_Gain
   * Referenced by: '<S299>/Gain2'
   */
  1125899907,

  /* Computed Parameter: Gain3_Gain
   * Referenced by: '<S303>/Gain3'
   */
  1125899907,

  /* Computed Parameter: RatedCurrent_Y0
   * Referenced by: '<S15>/RatedCurrent'
   */
  0U,

  /* Computed Parameter: RatedCurrent_Y0_a
   * Referenced by: '<S79>/RatedCurrent'
   */
  0U,

  /* Computed Parameter: RatedCurrent_Y0_b
   * Referenced by: '<S147>/RatedCurrent'
   */
  0U,

  /* Computed Parameter: RatedCurrent_Y0_g
   * Referenced by: '<S215>/RatedCurrent'
   */
  0U,

  /* Computed Parameter: M1_DCLinkCircuitVoltage_Y0
   * Referenced by: '<S305>/M1_DCLinkCircuitVoltage'
   */
  0U,

  /* Computed Parameter: M2_DCLinkCircuitVoltage_Y0
   * Referenced by: '<S305>/M2_DCLinkCircuitVoltage'
   */
  0U,

  /* Computed Parameter: M3_DCLinkCircuitVoltage_Y0
   * Referenced by: '<S305>/M3_DCLinkCircuitVoltage'
   */
  0U,

  /* Computed Parameter: M4_DCLinkCircuitVoltage_Y0
   * Referenced by: '<S305>/M4_DCLinkCircuitVoltage'
   */
  0U,

  /* Computed Parameter: M1_ActuralTorque_Y0
   * Referenced by: '<S305>/M1_ActuralTorque'
   */
  0,

  /* Computed Parameter: M2_ActuralTorque_Y0
   * Referenced by: '<S305>/M2_ActuralTorque'
   */
  0,

  /* Computed Parameter: M3_ActuralTorque_Y0
   * Referenced by: '<S305>/M3_ActuralTorque'
   */
  0,

  /* Computed Parameter: M4_ActuralTorque_Y0
   * Referenced by: '<S305>/M4_ActuralTorque'
   */
  0,

  /* Mask Parameter: CounterLimited_uplimit
   * Referenced by: '<S39>/FixPt Switch'
   */
  500U,

  /* Mask Parameter: CounterLimited_uplimit_l
   * Referenced by: '<S104>/FixPt Switch'
   */
  500U,

  /* Mask Parameter: CounterLimited_uplimit_lw
   * Referenced by: '<S172>/FixPt Switch'
   */
  500U,

  /* Mask Parameter: CounterLimited_uplimit_e
   * Referenced by: '<S240>/FixPt Switch'
   */
  500U,

  /* Computed Parameter: Output_InitialCondition
   * Referenced by: '<S30>/Output'
   */
  0U,

  /* Computed Parameter: Memory_InitialCondition
   * Referenced by: '<S14>/Memory'
   */
  0U,

  /* Computed Parameter: Constant2_Value_a
   * Referenced by: '<S14>/Constant2'
   */
  15U,

  /* Computed Parameter: Constant1_Value_dk
   * Referenced by: '<S14>/Constant1'
   */
  7U,

  /* Computed Parameter: Constant_Value_hu
   * Referenced by: '<S14>/Constant'
   */
  6U,

  /* Computed Parameter: FixPtConstant_Value
   * Referenced by: '<S38>/FixPt Constant'
   */
  1U,

  /* Computed Parameter: Constant_Value_js
   * Referenced by: '<S39>/Constant'
   */
  0U,

  /* Computed Parameter: Output_InitialCondition_i
   * Referenced by: '<S95>/Output'
   */
  0U,

  /* Computed Parameter: Memory_InitialCondition_m
   * Referenced by: '<S78>/Memory'
   */
  0U,

  /* Computed Parameter: Constant2_Value_ff
   * Referenced by: '<S78>/Constant2'
   */
  15U,

  /* Computed Parameter: Constant1_Value_jt
   * Referenced by: '<S78>/Constant1'
   */
  7U,

  /* Computed Parameter: Constant_Value_ll
   * Referenced by: '<S78>/Constant'
   */
  6U,

  /* Computed Parameter: FixPtConstant_Value_f
   * Referenced by: '<S103>/FixPt Constant'
   */
  1U,

  /* Computed Parameter: Constant_Value_ij
   * Referenced by: '<S104>/Constant'
   */
  0U,

  /* Computed Parameter: Output_InitialCondition_j
   * Referenced by: '<S163>/Output'
   */
  0U,

  /* Computed Parameter: Memory_InitialCondition_m1
   * Referenced by: '<S146>/Memory'
   */
  0U,

  /* Computed Parameter: Constant2_Value_fp
   * Referenced by: '<S146>/Constant2'
   */
  15U,

  /* Computed Parameter: Constant1_Value_ek
   * Referenced by: '<S146>/Constant1'
   */
  7U,

  /* Computed Parameter: Constant_Value_cw
   * Referenced by: '<S146>/Constant'
   */
  6U,

  /* Computed Parameter: FixPtConstant_Value_j
   * Referenced by: '<S171>/FixPt Constant'
   */
  1U,

  /* Computed Parameter: Constant_Value_jt
   * Referenced by: '<S172>/Constant'
   */
  0U,

  /* Computed Parameter: Output_InitialCondition_c
   * Referenced by: '<S231>/Output'
   */
  0U,

  /* Computed Parameter: Memory_InitialCondition_k
   * Referenced by: '<S214>/Memory'
   */
  0U,

  /* Computed Parameter: Constant2_Value_jx
   * Referenced by: '<S214>/Constant2'
   */
  15U,

  /* Computed Parameter: Constant1_Value_pr
   * Referenced by: '<S214>/Constant1'
   */
  7U,

  /* Computed Parameter: Constant_Value_gp
   * Referenced by: '<S214>/Constant'
   */
  6U,

  /* Computed Parameter: FixPtConstant_Value_d
   * Referenced by: '<S239>/FixPt Constant'
   */
  1U,

  /* Computed Parameter: Constant_Value_iv
   * Referenced by: '<S240>/Constant'
   */
  0U,

  /* Computed Parameter: Constant_Value_ju
   * Referenced by: '<S16>/Constant'
   */
  10,

  /* Computed Parameter: Constant_Value_f4
   * Referenced by: '<S80>/Constant'
   */
  10,

  /* Computed Parameter: Constant_Value_fy
   * Referenced by: '<S148>/Constant'
   */
  10,

  /* Computed Parameter: Constant_Value_bb
   * Referenced by: '<S216>/Constant'
   */
  10,

  /* Computed Parameter: M1_ModeofOperationDisplay_Y0
   * Referenced by: '<S305>/M1_ModeofOperationDisplay'
   */
  0,

  /* Computed Parameter: M2_ModeofOperationDisplay_Y0
   * Referenced by: '<S305>/M2_ModeofOperationDisplay'
   */
  0,

  /* Computed Parameter: M3_ModeofOperationDisplay_Y0
   * Referenced by: '<S305>/M3_ModeofOperationDisplay'
   */
  0,

  /* Computed Parameter: M4_ModeofOperationDisplay_Y0
   * Referenced by: '<S305>/M4_ModeofOperationDisplay'
   */
  0,

  /* Computed Parameter: Constant1_Value_l0
   * Referenced by: '<S18>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_hu
   * Referenced by: '<S19>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_it
   * Referenced by: '<S82>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_jd
   * Referenced by: '<S83>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_bv
   * Referenced by: '<S150>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_ma
   * Referenced by: '<S151>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_l4
   * Referenced by: '<S218>/Constant1'
   */
  true,

  /* Computed Parameter: Constant1_Value_lg
   * Referenced by: '<S219>/Constant1'
   */
  true
};

Robot_cal_type *Robot_cal = &Robot_cal_impl;

#endif                                 /*Robot_MDLREF_HIDE_CHILD_*/
